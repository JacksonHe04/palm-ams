...
const requireComponent = import.meta.glob('../views/**/*.vue') // 找到 modules 路径下的所有文件
const indexPathWhiteList = ['/', '/login', '/login','/loginsimple', '/lyterminal']
// 根路由文件白名单
const indexVuePathWhiteList = ['../views/login.vue','../views/lyterminal.vue','../views/loginsimple.vue','../views/index.vue','../views/error/404.vue']
// 路由数组作为Children使用
const childrenRouterList = [];
Object.keys(requireComponent).forEach(key => {
  // 路由名称
  const name = requireComponent[key].name;
  if(name.indexOf("/components/")==-1 && indexVuePathWhiteList.indexOf(name) == -1){
    let isSame = false
